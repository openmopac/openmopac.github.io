...
public class CmdExt extends ScriptExt {
  ...
  private static LabelToken labeler; // leave this like this in Legacy rather than a static reference for the legacy java2script 4.2 compiler
  ...
  public Object getBitsetIdentFull(BS bs, String label, boolean useAtomMap,
    ...
    Map<String, Object> htValues = (isAtoms || asIdentity ? null
        : labeler.getBondLabelValues());
    LabelToken[] tokens = (asIdentity ? null
        : isAtoms ? labeler.compile(vwr, label, '\0', null)
            : labeler.compile(vwr, label, '\1', htValues));
    ...
    for (int j = (haveIndex ? index : bs.nextSetBit(0)); j >= 0; j = bs
      ...
      if (isAtoms) {
        str = (asIdentity ? modelSet.at[j].getInfo()
            : labeler.formatLabelAtomArray(vwr, modelSet.at[j], tokens, '\0',
                indices, ptTemp));
      } else {
        ...
        str = (asIdentity ? bond.getIdentity()
            : labeler.formatLabelBond(vwr, bond, tokens, htValues, indices,
                ptTemp));
      }
      ...
      case T.hydrogen:
        ...
        if (!chk) {
          ...
          if (andBond) {
            if (bs1 == null)
              bs1 = vwr.bsA();
            vwr.makeConnections(0.1f, 1e8f, Edge.BOND_AROMATIC, T.modify, bs1,
  ...
  private void compare() throws ScriptException {
    ...
    for (int iFrame = 0, iModel = bsModels
      ...
      if (vwr.rotateAboutPointsInternal(eval, center, pt1,
          ...
          ptsB, null, null, false, centerAndPoints) && doAnimate && eval.isJS)
  ...
  private void mapProperty() throws ScriptException {
    ...
    while (true) {
      ...
      if (T.tokAttrOr(tokProp1, T.intproperty, T.floatproperty)
        ...
        float[] data1 = getBitsetPropertyFloat(bsFrom,
            tokProp1 | T.selectedfloat, null, Float.NaN, Float.NaN);
        float[] data2 = getBitsetPropertyFloat(bsFrom, tokKey | T.selectedfloat,
        ...
        float[] dataOut = new float[isProperty ? vwr.ms.ac : data3.length];
        bsOut = new BS();
        if (data1.length == data2.length) {
          ...
          if (isProperty)
            vwr.setData(property2, new Object[] { property2, dataOut, bsOut,
                Integer.valueOf(JmolDataManager.DATA_TYPE_AF), Boolean.TRUE },
      ...
      case T.path:
        ...
        if (getToken(i + 1).tok == T.dollarsign) {
          ...
          int indexEnd = (int) (isFloatParameter(i + 1) ? floatParameter(++i)
              : Integer.MAX_VALUE);
          list.addLast(
              new Object[] { Integer.valueOf(T.path), Float.valueOf(timeSec),
  ...
  private void polyhedra() throws ScriptException {
    ...
    for (int i = 1; i < slen; ++i) {
      ...
      case T.hash:
        propertyName = "info";
        propertyValue = ((SV) e.theToken).getMap();
        needsGenerating = true;
        break;
      case T.point:
        propertyName = "points";
        propertyValue = Float
            .valueOf(tokAt(++i) == T.off ? 0 : e.floatParameter(i));
        ok = true;
        break;
      case T.scale:
      ...
      case T.decimal:
        ...
        propertyValue = Float
            .valueOf(radius = (radius == 0 ? 0 : floatParameter(i)));
        needsGenerating = true;
        break;
      case T.offset:
        if (isFloatParameter(i + 1)) {
  ...
  private String write(T[] args) throws ScriptException {
    ...
    if (data == null) {
      ...
      if (data == "MENU") {
        ...
        data = vwr.ms.getPointGroupAsString(vwr.bsA(), null, 0, 1.0f, null,
  ...
  private void unitcell(int i, final boolean isModelkit) throws ScriptException {
    ...
    case T.surface:
      ...
      T3 hkl = eval.getPointOrPlane(i + 1, ScriptParam.MODE_P34);
      i = eval.iToken;
  ...
  private P3[] getUVBoxFromHKL(SymmetryInterface uc, P4 hkl, P4 plane)
    ...
    for (int i = cpts.size(); --i >= 0;) {
      ...
      if (d < 0.01f || v != null && ((da = Math
          .abs(Measure.computeTorsion(v, zero, plane, pt, true))) > -amin))
        continue;
      isLinear = (da < 1);
      if (v == null) {
        // load v
      ...
      if (isNew || (okAng = (da > 89)) && d < dmin + 0.1f
          && (pt.x >= -0.01f && pt.y <= 0.01f)
          && (damin == Float.MAX_VALUE || damin < 89)) {
  ...
  private boolean getUnitCellParameter(int i, Object[] ret)
    ...
    if (e.isArrayParameter(i)) {
      ...
      try {
          ret[0] = e.floatParameterSet(i, 6, 6);
        e.ignoreError = false;
        return false;
      } catch (Exception e) {
